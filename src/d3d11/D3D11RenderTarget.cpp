	...
	: _tex2D(tex)
{
	...
	dynamic_cast<D3D11Renderer*>(D3D11Renderer::Get())->GetDevice()->CreateRenderTargetView(_tex2D, 0, &_renderTarget);
}
	...
	, _tex2D(0)
{
	...
	dynamic_cast<D3D11Renderer*>(D3D11Renderer::Get())->GetDevice()->CreateTexture2D(&tex2DDesc, 0, &_tex2D);
	...
	dynamic_cast<D3D11Renderer*>(D3D11Renderer::Get())->GetDevice()->CreateRenderTargetView(_tex2D, &renderTargetDesc, &_renderTarget);
}
...
void D3D11RenderTarget::Clear()
{
	...
	dynamic_cast<D3D11Renderer*>(D3D11Renderer::Get())->GetDeviceContext()->ClearRenderTargetView(_renderTarget, clear);
}
