...
namespace Eternal
{
	namespace Graphics
	{
...
#if ETERNAL_USE_DEBUG_LAYER
		ID3D12Debug3*								D3D12WindowsDevice::_Debug3										= nullptr;
		IDXGIInfoQueue*								D3D12WindowsDevice::_DXGIInfoQueue								= nullptr;
		IDXGIDebug*									D3D12WindowsDevice::_DXGIDebug									= nullptr;
		ID3D12DeviceRemovedExtendedDataSettings*	D3D12WindowsDevice::_D3D12DeviceRemovedExtendedDataSettings		= nullptr;
		ID3D12DeviceRemovedExtendedDataSettings1*	D3D12WindowsDevice::_D3D12DeviceRemovedExtendedDataSettings1	= nullptr;
#endif
		bool										D3D12WindowsDevice::_IsInitialized								= false;
		IDXGIFactory4*								D3D12WindowsDevice::_DXGIFactory								= nullptr;

		void D3D12WindowsDevice::Initialize()
		{
			if (_IsInitialized)
				return;
		...
		void D3D12WindowsDevice::Destroy()
		{
			if (!_IsInitialized)
				return;
		...
		IDXGIFactory4* D3D12WindowsDevice::GetDXGIFactory()
		{
			return _DXGIFactory;
		}
			...
			: D3D12Device(InDeviceIndex)
		{
			...
			VerifySuccess(
				_DXGIAdapter1->QueryInterface(
					__uuidof(IDXGIAdapter),
					reinterpret_cast<void**>(&_DXGIAdapter)
				)
			);
			...
			VerifySuccess(
				_DXGIAdapter1->GetDesc1(&DXGIAdapterDesc1)
